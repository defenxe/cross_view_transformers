...
class ModelModule(pl.LightningModule):
    def __init__(self, backbone, loss_func, metrics, optimizer_args, scheduler_args=None, cfg=None):
        ...
        self.validation_step_outputs = [] # (기존 유지, 필요시 사용)
        self.inference_times = [] # 배치 단위 전체 처리 시간 저장을 위한 리스트
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # 1. GPU 동기화 및 시작 시간 기록 (전체 파이썬 루프 포함 시작)
        if torch.cuda.is_available():
            torch.cuda.synchronize()
        start_time = time.perf_counter()

        # 2. 추론, Loss 계산, Metric 업데이트 모두 수행 (shared_step 호출)
        ...
        # 4. 소요 시간 계산 (초 -> 밀리초 변환)
        # 이 시간은 '해당 배치 하나'를 엔드투엔드로 처리하는 데 걸린 시간입니다.
    ...
    def validation_epoch_end(self, outputs):
        # 5. 평균 처리 시간 계산 및 WandB/콘솔 로깅
        if self.inference_times:
            # 리스트에 쌓인 시간들은 '배치 1개당 처리 시간'의 모음이므로,
            # 그냥 평균을 내면 '배치 당 평균 추론(End-to-End) 시간'이 됩니다.
            avg_time_per_batch = sum(self.inference_times) / len(self.inference_times)
            
            # WandB 로깅 (배치 당 시간 ms)
            self.log('val/avg_time_per_batch_ms', avg_time_per_batch, prog_bar=True)
            
            # 터미널 출력
            print(f"\n[Epoch {self.current_epoch}] Average Time per Batch (End-to-End): {avg_time_per_batch:.2f} ms")
